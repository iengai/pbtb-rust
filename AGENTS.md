...
| Code style, comments, git/branch/commit rules; the CI agents, the local agent identity | [docs/conventions.md](docs/conventions.md) · [docs/agents.md](docs/agents.md) |
...
- 🔴 **Terraform / NAT egress.** In `terraform/envs/dev`, every apply is scoped with `-target`; the `guard-shell` hook refuses an unscoped apply and any destroy. The NAT instance is the **sole egress for all trading traffic** and the telebot host, with `user_data_replace_on_change = true`: any `user_data`/AMI change **destroys + recreates** it, blackholing trading egress and taking telebot down until the next telebot-deploy: a maintenance-window op **with trading quiesced first**. Any plan/apply needs every Lambda's `target/lambda/<bin>/bootstrap` to exist. The two ECR repos are already adopted into state; never let Terraform recreate them. → [docs/deployment/overview.md](docs/deployment/overview.md)
