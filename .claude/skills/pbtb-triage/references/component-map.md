...
| **passivbot bots** | ECS tasks on `scalable-cluster-dev-cluster`, one per bot, **task-def family per engine line**: `…-passivbot` (v7), `…-passivbot-v8` (v8). Launched by telebot Run and by the restart lambda via `RunTask` with `USER_ID`/`BOT_ID` overrides; a Restart is a `StopTask` with reason `restart requested` that the lambda relaunches. | Task-defs by scoped `terraform`, images by CodeBuild (`pbtb-deploy`) | Container logs: `/ecs/scalable-cluster-dev/passivbot` (v7) / `…/passivbot-v8` (v8), stream `passivbot/passivbot-container/<task-id>`. Memory: **Container Insights** (`ECS/ContainerInsights` `MemoryUtilized`, or the `…/performance` log, Type=Task) — `bot-status --memory`. | Task revision vs latest; **usage vs the latest revision's hard limit**; config `config_version` vs the family it runs on. | The ECS container host is **not SSM-managed** (`docker stats` impossible). A task keeps running on a deregistered/older revision until restarted — the limit change only bites at restart. |
| **restart lambda** `task-state-change-handler` | Lambda, triggered by ECS task state-change events. Reads bot desired state (DynamoDB), the bot's config (S3, for the engine line), relaunches enabled bots stopped by OOM or by a Restart (`stoppedReason` starts with `restart requested`) behind a CAS start lock. | `lambda-deploy` for code, scoped `terraform` for env/IAM (docs/deployment/lambda.md) | `/aws/lambda/scalable-cluster-dev-task-state-change-handler` (`pbtb_ops.py lambda-logs task-state`). `smoke-lambda task-state` invokes with an ignored event: 200 + no FunctionError proves the binary cold-starts and parses its env. | `CodeSha256` vs the last green `lambda-deploy`; env keys vs what the binary version expects (`deploy-audit`). | **Env and binary are coupled**: a terraform apply that changes env shape must be followed by `lambda-deploy` immediately (and vice-versa); in the window the old binary fails at config load and OOM restarts silently stop (a Restart then leaves the bot Enabled · Stopped). Mitigation pattern: temporarily add the old key back with `update-function-configuration`, then let terraform remove it. |
| **collector lambda** `daily-pnl-snapshot` | Lambda in VPC (egress via NAT EIP — Bybit keys are IP-whitelisted), EventBridge cron 01:00 UTC. | `daily-pnl-snapshot-deploy` (`pbtb-deploy`) | `/aws/lambda/scalable-cluster-dev-daily-pnl-snapshot`. Real run needs a `Scheduled Event` payload; anything else hits the guard and returns. | Chart files in `s3://scalable-cluster-dev-return-charts/charts/` refreshed today? `_state/` present per bot? `public/index.json` refreshed, one `public/bots/<id>.json` per showcase bot? | Bybit calls only work from the NAT EIP — a local probe returns `retCode 10010 Unmatched IP`; that is expected, not a key problem. Hyperliquid reads have no IP whitelist: public `POST https://api.hyperliquid.xyz/info` queries keyed by the account address, so a local probe works (take only `wallet_address` from `api-keys.json`, never print the file). A Hyperliquid bot is still skipped (`no api keys stored; skipping`) unless its `api-keys.json` holds both `wallet_address` and `private_key`. |
| **candle collector lambda** `hl-candle-collector` | Lambda **outside** the VPC (the candle query is public; no NAT), EventBridge cron 00:15 UTC. Stores each complete UTC day of Hyperliquid 1m candles still in the 5000-candle reach (~3.5 days) for every coin a `predefined/` Hyperliquid template approves. | `lambda-deploy -f target=hl-candle-collector` for code, scoped `terraform` (`terraform/envs/dev/hl-candles.tf`) for env/IAM | `/aws/lambda/scalable-cluster-dev-hl-candle-collector` (`pbtb_ops.py lambda-logs hl-candles`). A real run needs a `Scheduled Event` payload; anything else hits the guard and returns. An `ERROR` "gap is permanent" names a day that fell out of reach before it was stored. | Yesterday's `s3://scalable-cluster-dev-market-data/hyperliquid/1m/<COIN>/<YYYY-MM-DD>.json` present per coin? | A day missed for more than ~3 days cannot be refetched: Hyperliquid no longer serves it. The lab reads the bucket through `scripts/hl_candles_pull.py`. |
| **web console (site)** | GitHub Pages, source = **GitHub Actions** (`pages-publish.yml`, on push to main touching `site/`, and dispatch). Code and committed template backtests; the showcase (`/p`, `/p/bots/:id`, the live runs on `/configs/:name`) is fetched at run time from the showcase CDN over the chart bucket's `public/` prefix (docs/data-model.md); a tenant's own curves reach the browser through `GET /api/v1/bots/{id}/returns` after sign-in. | `pages-publish` (`pbtb-deploy`) | Workflow run logs; `curl https://iengai.github.io/pbtb-rust/templates/index.json`, `curl -sI https://d1rxu490g9jchv.cloudfront.net/index.json` for the showcase, `…/templates/audience.json` for the published templates (a retire the public catalogue does not show: the overlay's `published` still names it). | Pages `build_type` must be `workflow`. The workflow has no AWS access; the CDN reads the bucket through its origin access control alone (`showcase-cdn.tf`). | The old `gh-pages` branch is unused; pushing to it changes nothing. A stale *curve* is the collector, not the site. |
| **CI (GitHub Actions)** | `telebot-build` (push), `telebot-deploy`, `lambda-deploy` (restart lambda, mcp-http, candle collector), `daily-pnl-snapshot-deploy`, `pages-publish`, `incident-intake`, `incident-diagnose`, `issue-fix` (dispatch/schedule/issue label). | — | `gh run view <id> --log \| grep …`. OIDC roles per workflow; env-scoped jobs need `environment:<name>` in the role trust. | Secrets present (`gh secret list`); a `workflow_dispatch` workflow must exist on **main** to be dispatchable at all (404 otherwise). | A session dispatches as the local agent App (docs/agents.md § Local agents); when a dispatch fails to authenticate, `python scripts/ops/agent_identity.py status` names the check that does not hold. Builds use `rust:1.98-bookworm` (no symbol above GLIBC_2.34, exactly AL2023's glibc); the builder installs only `ca-certificates` — everything is rustls, do not re-add libssl-dev (its security-suite pool file 404s from GitHub's CDN edge). |
...
prints the PR in the log instead).
