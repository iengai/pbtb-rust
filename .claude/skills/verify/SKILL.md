...
| knowledge budgets (`AGENTS.md` ≤ 6 KB, `.claude/CLAUDE.md` ≤ 1.5 KB, `REVIEW.md` ≤ 4 KB, skill `description` ≤ 300 B; a `headroom` line within a tenth of a ceiling) | the always-loaded context is paid by every session, and a review policy is applied whole or not at all; docs/conventions.md § Knowledge placement says what to demote instead of growing it |
