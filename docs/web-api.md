...
| `POST /bots` | write | Add bot | `{name, exchange?, api_key, secret_key, overwrite?}` (`exchange` is `bybit`, the default, or `hyperliquid`, where `api_key` is the account address and `secret_key` an API wallet's private key; malformed credentials are a 400) → 201 `added`; 409 `already_exists` unless `overwrite: true` (then 200 `overwritten`: keys rotated, id and desired state kept) |
...
| `GET /templates` | read | Choose config | `{templates:[{name, title, exchange, min_vip_level, audience}]}`; a template applies to bots of its `exchange` only; nothing is hidden by level; `audience` is `everyone` (published) or `operator` (retired), and an `operator` one is listed to the operator's account only |
