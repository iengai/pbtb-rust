...
- A PR that changes behaviour updates the leaf, skill, or invariant that described the old behaviour (the PR template asks). One that removes a user-facing surface (a button, route, tool or dialogue) greps the root `README.md` and every `docs/` leaf for the surface's name and rewords or removes each mention.
...
- A change to the field set of a published JSON under `site/templates/` or `site/data/` greps `site/README.md` and every `docs/` leaf for that file's field list, and edits it in the same PR.
