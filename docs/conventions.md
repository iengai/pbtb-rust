...
- The "diagnose only" tier runs itself: the `incident-diagnose` workflow (`scripts/ops/diagnose_issue.py`) answers every Incident issue a collaborator opens or the intake files with a read-only diagnosis comment, from any OpenAI-compatible model (`LLM_BASE_URL` / `LLM_MODEL` variables, `LLM_API_KEY` secret). Its AWS view is the `gh-diagnose` role (`terraform/envs/dev/diagnose-ci.tf`): describe, list and logs only, bot rows without key attributes, no SSM. The comment is evidence for the human triage, not a verdict; the higher tiers still start from a person.
