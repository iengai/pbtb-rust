...
- The "diagnose only" tier runs itself: the `incident-diagnose` workflow (`scripts/ops/diagnose_issue.py`) answers every Incident issue a collaborator opens or the intake files with a read-only diagnosis comment, from any OpenAI-compatible model (`LLM_BASE_URL` / `LLM_MODEL` variables, `LLM_API_KEY` secret; `LLM_MODEL` may list several in order of preference and `LLM_FALLBACK_*` names a second endpoint, a failing model being dropped for the run). Its AWS view is the `gh-diagnose` role (`terraform/envs/dev/diagnose-ci.tf`): describe, list and logs only, bot rows without key attributes, no SSM. The comment is evidence for the human triage, not a verdict; a higher tier starts from a person, or from the diagnosis itself when the finding is self-evident (below).
- The "open a PR" tier runs on request: a collaborator adds the `agent:fix` label (or dispatches `issue-fix` by number) and `scripts/ops/fix_issue.py` works on a branch `fix/issue-<n>` with the same model, tools bounded to the checkout plus whitelisted cargo / gate commands, a deny list that keeps workflows, terraform, hooks, the three scripts behind the intake / diagnose / fix workflows, `.git`, `.cargo`, build scripts, dependency manifests, `AGENTS.md` and `REVIEW.md` human-owned, and no AWS credentials; the job that runs the model (and so the tests it writes) holds only a read-only token, its commands see neither that token nor the model keys, and the job that commits, pushes and opens the PR runs no model code. Only collaborators' comments on the issue reach the model. The field *How far the agent may go* is what grants the PR, not the label. A change under `src/` or `tests/` becomes a PR only after a `cargo test` was seen red and then green, any change only after a green gate; the PR carries the gate lines, the red→green evidence, a `REVIEW.md` pass by the same model and `closes #<n>`, and the `verify` workflow is dispatched on the branch (the `pull_request` run on a PR the repository token opens waits for a maintainer's approval, so the dispatched run is the one to read; the repository setting *Allow GitHub Actions to create and approve pull requests* must be on for that token to open one at all). At a merge tier the PR is armed to merge itself when `gate` passes (repository settings *Allow auto-merge* and *Automatically delete head branches*, and the ruleset requiring `gate`); a deploy stays with a person at every tier.
