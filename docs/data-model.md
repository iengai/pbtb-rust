...
- `predefined/` — reusable configuration templates. The key is the template's
  ...
  operator edits the object to change the level, no deploy needed. The
  audience is set from the console's Configs page, which rewrites the object
  (formatting and key order may change; what passivbot reads does not) and
  republishes the public catalogue's overlay (`public/templates/audience.json`
  below); an audience edited in the object directly reaches the public pages
  at the next switch, an unchanged one included, or template script run. Beside it, `lab` is the strategy
...
`public/` is the one prefix that leaves the bucket: the showcase CDN serves it (`terraform/envs/dev/showcase-cdn.tf`; the bucket's only policy, pinned to this prefix and to that distribution), and the site's showcase and catalogue pages read it from there. It holds `public/index.json` (every showcase bot: opaque id, name, exchange, link or `null`, current return, a 30-day sparkline) and `public/bots/{id}.json` (the curve as index and return per day, the config switches and capital resets each with the rounded capital the bot ran at, never a balance or a realized figure). `{id}` is `sha256("{user_id}#{bot_id}")` cut to twelve hex characters: a collision-safe key across tenants (bot ids are per-tenant names), not a shield for the `user_id`, which a keyed hash would be. The published set is every bot whose account row is the operator's and that is on the showcase (`showcase`, or with no choice made, `public_url`). Two writers keep the prefix to it, both in `infra::showcasestore` and both rebuilding `index.json` from the files present, with `Cache-Control: public, max-age=30`: the showcase switch (`PUT /api/v1/bots/{id}/showcase`) writes or removes that one bot's file as it saves the choice, and each collector run writes the day's files and removes the files of bots not published, deciding on the rows as it re-reads them at the end of the run.

`public/templates/audience.json` is the public catalogue's overlay: `{generated_at, published[template id]}`, the templates offered to everyone, exactly what `GET /api/v1/templates` lists a member. The site's catalogue pages read it over the committed backtest snapshot (`site/templates/index.json`): a snapshot template it names is published and one it does not is retired, and with no overlay, or none within 3 s, the snapshot's own `audience` holds. Two writers, both rebuilding it whole from `predefined/` with `Cache-Control: public, max-age=30`: the template switch (`PUT /api/v1/templates/{name}/audience`, `infra::templateaudiencestore`) after it saves, an unchanged audience included, and each template script that rebuilds the snapshot from S3 (`scripts/backtest_templates.py` and the scripts that call its `write_index`), so an overlay never outlives a newer snapshot. Two switches racing can each miss the other's save; the next switch repairs it.

`showcase/bots/{id}.json` is the same artifact for every bot on the operator's account, shown or not, written by each run; the switch publishes from it with the name and link overwritten from the row. It is private (no bucket policy names it; only the collector's and the API function's roles read it) and never deleted, so a bot recreated under the same id, or an account given the operator role back, publishes its last collected curve until the next run replaces it.
