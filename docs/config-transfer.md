...
|---|---|---|
| `universe` | `mix3`, `mix8`, `mix10`, `mix12`, `mix17`, `mix24`, `alt10`, `xrp` | the backtest's coins |
| `capital_usdt` | `100` … `10000` | the backtest's starting balance: the least capital the tuning is offered for. Not a promise that a larger balance behaves the same: the site's capital profile (site/README.md `capital_profile`) shows each balance's gain, drawdown and traded coins |
...
| `profile` | `guard` (≤12%), `steady` (≤20%), `balanced` (≤28%), `bold` (≤40%), `extreme` | the tier the worst drawdown across the strategy lab's robustness runs falls in, by the lab's bands (`strategy_lab/scripts/round22_accept.py` `BANDS`): the full-window backtest, the jittered parameters and the leave-one-coin-out runs at `capital_usdt`, and the capital scan at every balance from it up (`lab.notes` quotes them), so the site's full-window drawdown can sit in a lower tier than the profile; a template that predates the lab keeps the tier measured when it was published (under the earlier <10% / <25% / <35% / <60% bands); a Hyperliquid copy keeps its Bybit template's tier, since its own hourly run overstates the drawdown (its page shows both); one tuning on both engine lines keeps the more cautious |
