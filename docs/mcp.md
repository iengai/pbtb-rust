...
| `list_templates` | `bots:read` | `{name, title, min_vip_level, audience}` each; nothing is hidden by level; a template whose `audience` is `operator` (retired) is listed to the operator's account only |
