//! S3 IO for the collector's private halves, keyed by tenant and bot: the
//! series the API serves to its owner (under the configured prefix, at the key
//! `infra::returncurverepository::series_key` names) and the accumulated ledger
//! state under `_state/`, which only the collector reads. The showcase halves,
//! shared with the API, are `infra::showcasestore`.
